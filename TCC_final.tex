...
\bottomrule
\end{tabular}
\caption*{\footnotesize Observações em fundo(-ativo)-mês. A Seção~\ref{sec:resultados} apresenta
os resultados na mesma ordem desta tabela: as estimativas e os erros estáticos (Etapas 1--2) usam
a amostra inicial (250 fundos, só VALE3); os erros fora da amostra, na forma dinâmica (Etapa 3
generalizada), usam a amostra amplamente expandida (última linha).}
\end{table}
...
A exigência simultânea das cinco características do fundo (Seção~\ref{sec:metodologia})
reduz a cobertura temporal de 72 para 59 meses efetivos (fevereiro de 2017 a dezembro de 2021):
uma delas, o beta do fundo, precisa de pelo menos 252 pregões de histórico de cota (cerca de um
ano) para ser calculada, o que deixa os primeiros doze meses da amostra sem fundos suficientes.
Essa perda de cobertura inicial foi uma escolha deliberada, não uma limitação inesperada do
dado: preferiu-se um conjunto menor de meses com as cinco características completas a um
conjunto maior com características faltantes preenchidas de forma arbitrária.
...
o pseudo-$R^2$ de McFadden de cada regressão mensal.}
\end{figure}

\subsection{Erros --- estática (Etapas 1--2)}

\begin{table}[h]\centering\small
\caption{Estatísticas do erro $e$, Etapa 1 (8.335 observações).}
\begin{tabular}{@{}lr@{}}
\toprule
Estatística & Valor \\
\midrule
...
azul = média mensal (mecanicamente zero); linha vermelha tracejada = desvio-padrão mensal.}
\end{figure}
...
\caption{Evolução mês a mês do erro $u$.}
\end{figure}

\begin{table}[h]\centering\small
\caption{RMSE e MAE dentro e fora da amostra, ajuste parcial vs. ingênua (VALE3--Itaú).}
\begin{tabular}{@{}lrrr@{}}
\toprule
...
início do teste (2020-01).}
\end{figure}

\subsection{Erros --- dinâmica fora da amostra (Etapa 3 generalizada)}
...
(Tabelas~\ref{tab:rmse-gestora} e~\ref{tab:margem-gestora}).}
...
essa vantagem é estável ao longo do tempo.}
...
sistematicamente mais do que o modelo prevê.}
...
(RMSE) entre horizontes mostrada na Figura a seguir.}
...
($h=1$ vs.\ $h=12$, os mais distantes) a $0{,}99$ ($h=6$ vs.\ $h=12$, os mais próximos).}
...
--- em 4 horizontes, ordenado do mais ao menos errante em $h=1$.}
