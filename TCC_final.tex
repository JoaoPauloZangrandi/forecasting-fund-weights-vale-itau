...
outro?} A persistência testada até aqui foi \emph{entre gestoras} --- uma gestora com viés ou
dispersão alta continua assim ao longo do tempo. Falta perguntar o mesmo dentro de um único
fundo, mês a mês: se um fundo errou numa direção este mês, ele tende a errar na mesma direção no
mês seguinte? A figura a seguir testa isso e encontra que não --- a correlação é essencialmente
zero.
