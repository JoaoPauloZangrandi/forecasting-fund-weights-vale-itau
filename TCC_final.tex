...
ranking de dificuldade de previsão por gestora tem correlação de Spearman de $1{,}0000$ entre as
...
até o final desta subseção, todas as tabelas e figuras usam a base multiativo} (2.343 fundos,
41 gestoras, 501 ativos) --- não mais a amostra VALE3--Itaú das seções anteriores.

\paragraph{Um filtro adicional de qualidade de dado, específico desta análise de erro.} A
Tabela~\ref{tab:amostra} (Seção~\ref{sec:dados}) reporta 2.347 fundos e 8.268.920 observações
para a amostra final multiativo --- 4 fundos e 44.072 observações a mais do que os números
usados nesta subseção. A diferença vem de um problema encontrado ao investigar, em detalhe, o
fundo com maior RMSE fora da amostra que este trabalho havia identificado (a versão anterior da
Figura~\ref{fig:trajetoria-fundo}, adiante): a soma das participações de \emph{todas} as posições
declaradas por aquele fundo, num único mês da CDA da CVM, chegava a quase 500\% do patrimônio ---
confirmado na fonte bruta, não é erro deste trabalho, e concentrado no período em que o fundo
estava em liquidação acelerada (patrimônio caindo de R\$59 milhões para R\$1,6 milhão em seis
meses). Verificando a base inteira, esse tipo de inconsistência (soma de participações acima de
105\% do patrimônio) aparece em 597 dos 85.788 pares fundo-mês (0,7\%), espalhados por 73 fundos
--- raro, mas real, e capaz de distorcer o erro calculado para os fundos afetados. Esses
fundo-meses foram excluídos \textbf{antes} de qualquer regressão ou cálculo de erro (Etapa 1 e
Etapa 3 recalculadas do zero sobre a base já limpa), o que reduz o RMSE agregado fora da amostra
em $4{,}45\%$ ($h=1$) e $3{,}86\%$ ($h=3$) --- uma correção real, não cosmética, embora o ranking
de dificuldade por gestora praticamente não mude (correlação de Spearman de $0{,}97$ entre o
ranking antes e depois da limpeza). O caso mais afetado, disparado, é a Vinci Partners: RMSE
($h=1$) cai $44\%$ com a limpeza, puxado quase inteiramente pelo fundo problemático. Uma
auditoria mais ampla das duas bases (pesos negativos ou $>1$, AUM/cotistas ausentes ou
negativos, beta do fundo fora de um intervalo razoável, linhas duplicadas) não encontrou outro
problema dessa magnitude; o único ponto adicional --- fluxo de captação/resgate acima de $200\%$
do patrimônio no mesmo mês, em $74$ fundos ($0{,}06\%$ das observações, tipicamente fundos quase
zerados por resgates que somem quase todo o patrimônio dentro do próprio mês) --- muda o RMSE
agregado em apenas $0{,}09\%$ se excluído, e por isso não foi removido.
...
(Tabelas~\ref{tab:rmse-gestora} e~\ref{tab:margem-gestora}). Base: multiativo (2.343 fundos,
...
$h=3$ meses. Base: multiativo (2.343 fundos, 41 gestoras, 501 ativos).}
...
essa vantagem é estável ao longo do tempo. Base: multiativo (2.343 fundos, 41 gestoras,
501 ativos).}
...
\caption{Peso real vs. peso previsto pelo ajuste parcial ($h=3$), fundo 396303 (Guepardo
Investimentos), em JSLG3 --- o fundo com maior RMSE fora da amostra na base já limpa (nota
acima). O modelo não antecipa o desmonte de uma posição concentrada: o fundo tinha $69{,}5\%$ do
patrimônio em JSLG3 em jan/2020 e reduziu para $3{,}8\%$ já em fev/2020, patamar em que
permaneceu; baseado no peso elevado observado em jan/2020, o ajuste parcial previa continuidade
em torno de $60\%$ para abr/2020, quando o peso real já estava perto de zero havia meses. Base:
multiativo (2.343 fundos, 41 gestoras, 501 ativos) --- este gráfico exibe apenas um fundo e um
ativo (JSLG3) dessa base, para ilustração.}
...
sistematicamente mais do que o modelo prevê. Base: multiativo (2.343 fundos, 41 gestoras,
501 ativos).}
\label{fig:vies-dinamico}
\end{figure}

Esta parte final da subseção segue uma cadeia de perguntas, cada uma motivada pela resposta da
anterior. A Figura~\ref{fig:vies-dinamico} mostrou que algumas gestoras compram (ou vendem)
sistematicamente mais do que o modelo prevê --- um viés. Daqui em diante, nessa ordem: esse
viés é um traço real da gestora, ou coincidência da janela de teste? Se é real, dá pra corrigir
a previsão com base nele? Por que essa correção ajuda tão pouco? A parte do erro que sobra --- a
variância --- também é um traço estável da gestora, ou é ruído puro? O que explica essa
variância? E, por fim, esses achados valem só para o horizonte de um mês, ou se mantêm ao tentar
prever 3, 6 ou 12 meses à frente?
...
metades, a correlação é $0{,}80$ ($R^2=0{,}63$, $p<0{,}0001$): o viés é, de fato, persistente.
...
observações em cada metade. Base: multiativo (2.343 fundos, 41 gestoras, 501 ativos).}
...
estimativa), o RMSE agregado da 2ª metade cai de $0{,}005705$ para $0{,}005701$ --- uma melhora
de apenas $0{,}07\%$. A correção ajuda mais nas gestoras com viés mais extremo (Guepardo
Investimentos, $-3{,}5\%$ de RMSE; Real Investor, $-3{,}1\%$; Núcleo Capital, $-3{,}1\%$), mas é
irrelevante para a maioria. A razão fica clara na decomposição a seguir.

\paragraph{Por que o viés importa pouco: a variância domina o erro.} Pense em dois arqueiros
atirando num alvo, onde ``erro'' é a distância de cada flecha ao centro. O primeiro erra pouco,
mas sempre um pouco para o mesmo lado --- um desvio sistemático, fácil de corrigir (basta mirar
um pouco mais para o outro lado). O segundo acerta o centro \emph{em média}, mas cada flecha cai
num lugar diferente --- imprevisibilidade genuína, sem direção fixa, que nenhum ajuste simples
corrige. Os dois podem ter o mesmo erro médio ao quadrado, por motivos opostos: o primeiro por
\textbf{viés}, o segundo por \textbf{variância}. A identidade
$\text{RMSE}^2 = \text{viés}^2 + \text{variância}$ --- a mesma relação algébrica por trás da
fórmula usual de variância, $\text{Var}(X) = E[X^2] - E[X]^2$ --- separa essas duas fontes de
erro para cada gestora, e permite medir exatamente qual das duas pesa mais.
...
($h=1$), por gestora, ordenada por RMSE. Base: multiativo (2.343 fundos, 41 gestoras,
501 ativos).}
\label{fig:decomposicao-vies-variancia}
\end{figure}

Em média entre as 40 gestoras, o viés$^2$ explica só $0{,}65\%$ do $\text{RMSE}^2$ (mediana
$0{,}15\%$); mesmo a Guepardo, a mais enviesada, chega a apenas $5{,}70\%$. Somando todas as
...
correlação de $0{,}85$ ($R^2=0{,}71$, $p<0{,}0001$) --- quase tão persistente quanto o viés.
...
(2.343 fundos, 41 gestoras, 501 ativos).}
...
cabe perguntar o que a determina. As características de cada gestora usadas a seguir --- beta
médio do fundo, tamanho médio (log do PL) e concentração média da carteira (índice HHI dos pesos
por ativo, dentro de cada fundo-mês) --- são calculadas \textbf{apenas com dados do período de
treino} (anteriores a janeiro de 2020), genuinamente predeterminadas em relação ao desvio-padrão
do erro (medido só no período de teste): a mesma disciplina de defasagem usada em todo o resto
deste trabalho, para não misturar a caracterização com o próprio período que ela tenta explicar.
Cruzando essas três características --- as mais plausíveis à primeira vista --- contra o
desvio-padrão do erro, a concentração se destaca claramente: correlação de $0{,}68$
($R^2=0{,}47$), contra $0{,}47$ ($R^2=0{,}22$) do beta e apenas $0{,}14$ ($R^2=0{,}02$) do
tamanho.
...
Essas três correlações são bivariadas --- cada característica sozinha contra a dispersão do
erro --- e não dizem se elas têm efeito \emph{independente} umas das outras, ou se estão só
capturando a mesma coisa por tabela (gestoras com beta alto podem também ser, coincidentemente,
mais concentradas). Para isolar o efeito de cada característica, mantendo as outras fixas, a
concentração entra numa regressão junto com \textbf{todas as cinco características da Etapa~1}
--- não só beta e tamanho --- para não deixar de fora candidatos plausíveis (cotistas, indicador
de FIC, fluxo de captação/resgate) só porque a comparação inicial (Figuras~\ref{fig:beta-erro}
e~\ref{fig:explica-variancia}) partiu de beta. Como nas correlações acima, todas as seis
características são calculadas só com dados do período de treino (anteriores a janeiro de 2020)
--- predeterminadas em relação ao $\text{dp}(\text{erro})_g$, medido só no período de teste:
\begin{equation}
...
\beta_6\,\overline{\text{HHI}}_g + u_g
\label{eq:regressao-variancia}
\end{equation}

\begin{table}[h]\centering\small
\caption{Regressão múltipla (equação~\eqref{eq:regressao-variancia}): desvio-padrão do erro fora
da amostra por gestora ($h=1$, período de teste) explicado pelas cinco características da
Etapa 1 e pela concentração média, todas medidas só no período de treino. 40 gestoras.}
\label{tab:regressao-variancia}
\begin{tabular}{@{}lrrc@{}}
\toprule
Variável & Coeficiente & $t$ & Sig. \\
\midrule
Intercepto ($\alpha$) & $-0{,}0147$ & $-1{,}31$ & n.s. \\
Beta médio do fundo ($\beta_1$) & $0{,}0018$ & $0{,}71$ & n.s. \\
Tamanho médio, log AUM ($\beta_2$) & $0{,}0010$ & $1{,}56$ & n.s. \\
Cotistas médio, log ($\beta_3$) & $0{,}00003$ & $0{,}05$ & n.s. \\
\% FIC ($\beta_4$) & $-0{,}0007$ & $-0{,}38$ & n.s. \\
Fluxo/AUM médio ($\beta_5$) & $-0{,}0095$ & $-1{,}42$ & n.s. \\
Concentração média, HHI ($\beta_6$) & $0{,}0845$ & $4{,}18$ & *** \\
\midrule
$R^2$ & \multicolumn{3}{r}{$0{,}54$} \\
\bottomrule
\end{tabular}
\caption*{\footnotesize Variável dependente: desvio-padrão do erro fora da amostra por gestora
($h=1$). $^{***}p<0{,}1\%$; n.s. = não significativo a $5\%$.}
\end{table}

O resultado responde à pergunta: mesmo com as cinco características da Etapa 1 todas presentes,
e todas genuinamente defasadas em relação ao erro que tentam explicar, só a concentração
($\beta_6$) continua estatisticamente significativa --- beta e tamanho, que pareciam relevantes
sozinhos (Figura~\ref{fig:explica-variancia}), perdem significância quando a concentração entra
na mesma regressão; cotistas, o indicador de FIC e o fluxo de captação/resgate ficam bem longe
de significativos. Isso sugere que a relação entre beta e RMSE mostrada na
Figura~\ref{fig:beta-erro} a seguir é, em boa parte, um reflexo indireto da concentração ---
gestoras com fundos de beta alto tendem também a manter carteiras mais concentradas, e é a
concentração, não o beta em si, que parece determinar o quão imprevisível é a mudança de
posição.

\paragraph{Concentração no nível fundo-mês: sobretudo um traço entre fundos, não dentro do
fundo.} O resultado acima usa uma única média de concentração por gestora --- 40 pontos --- e por
isso não distingue duas perguntas bem diferentes. Pense em alunos de uma turma, horas de estudo
por semana vs.\ nota da prova daquela semana: jogando os dados de \emph{todos} os alunos numa
nuvem só, aparece uma correlação forte, mas em boa parte porque existe um \emph{tipo} de aluno
--- o dedicado estuda muito toda semana e tira nota alta toda semana; o relapso, o oposto. Isso
é uma diferença \emph{entre} alunos, não diz nada sobre uma semana específica. A pergunta
diferente é: pegando um único aluno, nas semanas em que \emph{ele} estudou um pouco mais do que
é normal para \emph{ele mesmo}, tirou uma nota um pouco melhor do que é normal para ele mesmo?
Essa é uma pergunta sobre variação \emph{dentro} do mesmo aluno, e pode ter uma resposta bem
diferente da correlação geral. Refinando o resultado da concentração para o nível fundo-mês ---
a concentração da carteira de um fundo específico, num mês específico, contra o erro desse mesmo
fundo nesse mesmo mês, com dezenas de milhares de pares em vez de 40 --- é possível separar as
duas perguntas em três versões.
...
por decil. Base: multiativo (2.343 fundos, 41 gestoras, 501 ativos).}
\label{fig:concentracao-mes}
\end{figure}

Nas versões contemporânea e defasada (painéis A e B), a correlação é praticamente idêntica
($0{,}41$ e $0{,}43$) --- a concentração do mês \emph{anterior}, que já era conhecida antes do
...
isolando a variação temporal de um mesmo fundo em relação a si mesmo) mostra uma correlação quase
nula ($0{,}06$): a concentração é, quase inteiramente, um traço que diferencia fundos \emph{entre
si} --- um fundo tipicamente mais concentrado do que a média tende a errar mais do que a média
--- e não um sinal de curto prazo de que um fundo específico está prestes a errar mais do que o
normal dele mesmo.
...
\caption{Beta médio do fundo (Etapa 1) vs. RMSE fora da amostra ($h=1$), 1.941 fundos com pelo
...
\emph{fora da amostra}: $R^2=0{,}24$. Base: multiativo (2.343 fundos, 41 gestoras, 501 ativos)
--- o beta do fundo usado aqui é o mesmo conceito estimado na Etapa 1, recalculado nessa base
maior.}
\label{fig:beta-erro}
\end{figure}

\paragraph{Uma pergunta diferente: o erro de um fundo específico se repete de um mês para o
outro?} As persistências testadas até aqui (viés e variância) comparam o comportamento
\emph{médio} de uma gestora numa metade do período de teste (cerca de 11 meses) contra a outra
--- um traço de longo prazo, como perguntar se um jogador de futebol abaixo da média no primeiro
turno de um campeonato continua abaixo da média no segundo turno. A pergunta agora é bem
diferente, e olha para um único fundo, mês a mês: depois de errar numa direção \emph{neste} mês,
esse fundo tende a errar na mesma direção \emph{no mês seguinte} --- uma ``fase ruim'' que se
repete de um mês para o outro, como um jogador que, depois de uma partida ruim, tende a jogar mal
na partida seguinte também? A figura a seguir testa isso e encontra que não --- a correlação é
essencialmente zero. As duas coisas não se contradizem: um fundo pode ser, de fato, geralmente
mais difícil de prever (um traço estável de longo prazo, já mostrado acima) sem que um mês ruim
específico avise nada sobre o mês seguinte.
...
erro médio no mês $t+1$ (agregado por fundo-mês, 33.600 pares consecutivos, 1.917 fundos), e
distribuição da autocorrelação lag-1 entre os 1.636 fundos com pelo menos 8 pares. Correlação
pooled $=0{,}015$; autocorrelação média entre fundos $=-0{,}077$ --- sem evidência de
persistência mês a mês na \emph{direção} do erro, distinto da estabilidade de \emph{magnitude}
(RMSE) entre horizontes mostrada na Figura a seguir. Base: multiativo (2.343 fundos, 41 gestoras,
501 ativos).}
\label{fig:persistencia-erro}
\end{figure}

\paragraph{Os achados valem só para $h=1$, ou se mantêm em horizontes mais longos?} Tudo até aqui
usou $h=1$ (previsão de um mês à frente). A figura a seguir verifica se o ranking de dificuldade
por gestora --- quem é mais difícil de prever --- muda quando o horizonte de previsão aumenta
para 3, 6 ou 12 meses.
...
($h=1$ vs.\ $h=12$, os mais distantes) a $0{,}98$ ($h=6$ vs.\ $h=12$, os mais próximos). Base:
multiativo (2.343 fundos, 41 gestoras, 501 ativos).}
\label{fig:ranking-horizontes}
\end{figure}

As duas tabelas a seguir trazem os números completos por trás da figura acima: RMSE e margem de
todas as 41 gestoras, nos quatro horizontes.
...
--- em 4 horizontes, ordenado do mais ao menos errante em $h=1$. Base: multiativo (2.343 fundos,
41 gestoras, 501 ativos).}
...
a ingênua. Base: multiativo (2.343 fundos, 41 gestoras, 501 ativos).}
...
Por fim, a figura a seguir junta as duas dimensões discutidas nesta subseção --- dificuldade
absoluta (RMSE) e se o ajuste parcial realmente ajuda (margem) --- num único gráfico, uma
gestora por ponto.
...
Base: multiativo (2.343 fundos, 41 gestoras, 501 ativos).}
\end{figure}
