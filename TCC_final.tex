...
claramente: correlação de $0{,}71$ ($R^2=0{,}50$), contra $0{,}52$ ($R^2=0{,}27$) do beta e
apenas $0{,}17$ ($R^2=0{,}03$) do tamanho.
...
As três correlações acima são bivariadas --- cada característica sozinha contra a dispersão do
...
mais concentradas). Para isolar o efeito de cada característica, mantendo as outras duas fixas,
as três entram juntas numa única regressão linear, por gestora ($g=1,\ldots,40$):
\begin{equation}
\text{dp}(\text{erro})_g = \alpha + \beta_1\,\overline{\text{beta}}_g +
\beta_2\,\overline{\log(\text{AUM})}_g + \beta_3\,\overline{\text{HHI}}_g + u_g
\label{eq:regressao-variancia}
\end{equation}

\begin{table}[h]\centering\small
\caption{Regressão múltipla (equação~\eqref{eq:regressao-variancia}): desvio-padrão do erro fora
da amostra por gestora ($h=1$) explicado por beta médio, tamanho médio e concentração média,
juntos. 40 gestoras.}
...
\midrule
Intercepto ($\alpha$) & $-0{,}0098$ & $-0{,}87$ & n.s. \\
Beta médio do fundo ($\beta_1$) & $0{,}0016$ & $0{,}69$ & n.s. \\
Tamanho médio, log AUM ($\beta_2$) & $0{,}0007$ & $1{,}16$ & n.s. \\
Concentração média, HHI ($\beta_3$) & $0{,}0961$ & $4{,}45$ & *** \\
\midrule
$R^2$ & \multicolumn{3}{r}{$0{,}53$} \\
\bottomrule
\end{tabular}
...
O resultado responde à pergunta: só a concentração ($\beta_3$) continua estatisticamente
significativa depois de controlar pelas outras duas --- beta e tamanho, que pareciam relevantes
sozinhos (Figura~\ref{fig:explica-variancia}), perdem significância quando a concentração entra
na mesma regressão. Isso sugere que a relação entre beta e RMSE mostrada na
