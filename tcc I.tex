...
para 88 a 98\% de um universo ampliado de 375 ações mantidas por fundos do Itaú --- embora o
efeito de tamanho da própria VALE3 esteja entre os mais extremos dessa distribuição, um caso
atípico, não representativo. O trabalho estima a velocidade de ajuste da carteira ao peso desejado por seis especificações de
...
that generalize, in the same direction, to 88--98\% of an expanded universe of 375 stocks
held by Itaú funds --- although Vale's own size effect sits among the most extreme of that
cross-asset distribution, an atypical rather than representative case. The paper estimates the
...
universo de ações que os fundos do Itaú carregam como posição direta --- 375 papéis com amostra
...
painel fundo$\times$ativo$\times$mês para as posições em ações dos fundos do Itaú. Um cuidado de
limpeza foi decisivo aqui: o campo \texttt{Nome\_Ativo} da CVM não é ``um ativo, uma linha'' --- a
mesma ação pode aparecer em até cinco linhas distintas por mês, conforme o tipo de custódia
(posição direta; cedida em empréstimo de ações; recebida em empréstimo; direito ou recibo de
subscrição; certificado de depósito), sem que isso signifique cinco ativos diferentes. Do
1.217 valores distintos de \texttt{Nome\_Ativo} no painel bruto (já sem as posições em empresas de
capital fechado da Seção anterior), apenas \textbf{464} correspondem a uma posição direta
padrão (identificados pelo sufixo numérico do \emph{ticker} --- 3, 4 a 8 ou 11, a convenção da B3
para ações ordinárias, preferenciais e \emph{units} --- combinado com a ausência de palavras como
``cedido em empréstimo'' ou ``subscrição'' no nome); as demais 753 linhas de \texttt{Nome\_Ativo}
(62\% do total, 40,6\% das observações fundo-ativo-mês) são variantes de custódia ou instrumentos
relacionados, não ações adicionais. Manter apenas a posição direta reproduz exatamente a
convenção já usada para VALE3 (Seção~\ref{sec:dados} usa só \texttt{Nome\_Ativo == "VALE ON N1 -
VALE3"}, nunca a variante emprestada), o que garante consistência metodológica entre o estudo de
caso e a extensão. Após esse filtro e o de mais de três cotistas, o painel final tem 894.551
observações de 305 fundos em 444 ativos. A padronização das características de escala nesse
painel usa a distribuição de patrimônio e de cotistas entre pares (fundo, mês) \emph{únicos}, não
entre linhas (fundo, ativo, mês) --- um fundo que detém cinquenta ações não deve pesar cinquenta
vezes mais do que um fundo que detém uma só na definição da média e do desvio-padrão usados para
...
Antes de prever, examina-se como os coeficientes da regressão mês a mês evoluem no tempo. A
Tabela~\ref{tab:dyn} resume, para cada série de coeficientes, a autocorrelação de primeira ordem,
o parâmetro $\phi$ do processo autorregressivo e a estatística do teste de Dickey--Fuller.
...
\begin{tabular}{@{}lrrrl@{}}
\toprule
Série (coeficiente) & Autocorrelação & $\phi$ (AR) & Dickey--Fuller & Leitura \\
\midrule
...
\bottomrule
\end{tabular}
\caption*{\footnotesize Valor crítico do teste de Dickey--Fuller a 5\% $\approx-2{,}9$; abaixo
dele, rejeita-se a raiz unitária. Fonte: elaboração própria (script \texttt{R/16}).}
\end{table}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{fig_theta_padr.pdf}
\caption{Coeficientes padronizados da regressão mês a mês, 2016--2021. A linha tracejada marca a
média de cada série.}
\label{fig:theta}
\caption*{\footnotesize Fonte: elaboração própria (script \texttt{R/29}).}
\end{figure}

Os coeficientes ficam com autocorrelação entre $0{,}37$ e $0{,}84$, claramente abaixo de um; o
teste de Dickey--Fuller rejeita a raiz unitária em quatro das cinco séries, a exceção sendo a de
cotistas, no limite ($-2{,}70$). A Figura~\ref{fig:theta} mostra o porquê visualmente: os painéis
de tamanho, cotistas e fundo de cotas nunca trocam de sinal ao longo dos 72 meses, mas oscilam
com uma reconfiguração visível em torno de 2018, período em que o preço e o beta de VALE3 também
se moveram de forma acentuada. O painel de fluxo, o menos persistente, oscila em torno de zero
com um único movimento extremo isolado --- coerente com sua não significância na
Seção~\ref{sec:det}. Essa persistência-com-reversão é o que legitima tratar os coeficientes como
um ``estado latente'' que evolui devagar, e é a evidência que justificou a correção de
Newey--West (Seção~\ref{sec:nw}).

\subsection{Previsão e a matriz de erros}

Comparam-se quatro previsões do peso, avaliadas fora da amostra com janela de estimação
expandida: a ingênua (M0, repete o último peso), o efeito fixo (M1, média histórica do fundo), o
fator com Kalman (M2, soma ao efeito fixo o desvio previsto pelas características) e a variação
(M3, prevê diretamente a mudança do peso). A Tabela~\ref{tab:fc} traz o RMSE em dois universos
(todos os fundos; só fundos de ações) e dois horizontes ($h=1$; $h=3$, o da opacidade).

\begin{table}[h]\centering\small
\caption{Erro de previsão fora da amostra (RMSE), por universo, horizonte e modelo.}
\label{tab:fc}
...
\bottomrule
\end{tabular}
\caption*{\footnotesize Menor RMSE em negrito por linha. Fonte: elaboração própria (scripts
\texttt{R/19}, \texttt{R/21}).}
\end{table}

A leitura é nítida: \textbf{a previsão ingênua vence em todas as oito células}, com os modelos
estruturais errando de 54\% a 63\% mais em $h=1$. O resultado mais informativo é o efeito fixo
(M1) \emph{perder} da ingênua: se o peso revertesse à média histórica do próprio fundo, M1
venceria; perder significa que não há reversão detectável ao nível do fundo no curto prazo --- a
assinatura de um processo próximo do martingale. O modelo de variação (M3) empata com a ingênua,
coerente: prever variação nula equivale a repetir o último peso. Restringir a fundos de ações
(posições maiores e mais ativamente geridas) amplia as diferenças sem mudar a ordem. Essa
persistência quase martingale é exatamente o que um $\lambda$ pequeno no ajuste parcial
(Seção~\ref{sec:paest}) prediz no limite --- a ponte entre esta seção e a seguinte.

\subsection{As duas margens}
\label{sec:margens}

Por fim, separa-se a decisão de ter a ação da decisão de quanto carregar. A margem extensiva é
estimada num painel que inclui os fundos de ações que não detêm VALE3, com peso zero: 10.286
observações de 260 fundos de ações, das quais $86\%$ têm a ação, com 75 entradas e 100 saídas no
período. O modelo logístico $\Pr(\text{tem VALE3})=\Lambda(\gamma_0+\gamma_1\ln PL+\gamma_2\ln
Cot+\gamma_3 FIC)$ dá os coeficientes da Tabela~\ref{tab:posse}.

\begin{table}[h]\centering\small
\caption{Modelo logístico da probabilidade de deter VALE3 (painel extensivo, 10.286 obs.).}
\label{tab:posse}
\begin{tabular}{@{}lrrrc@{}}
\toprule
...
\bottomrule
\end{tabular}
\caption*{\footnotesize Fonte: elaboração própria (script \texttt{R/20}).}
\end{table}
...
O achado descritivo mais interessante do trabalho: \textbf{as duas margens têm sinais opostos}.
Fundos maiores e fundos de cotas são mais propensos a ter a ação, mas, quando a têm, carregam um
peso menor --- a assinatura de diversificação: à medida que o fundo cresce ou acessa o ativo de
forma indireta, tende a incluir mais ativos, cada um com fatia menor. Quanto à previsão, a posse
muda em apenas $1{,}3\%$ dos meses, e a regra ingênua acerta $98{,}7\%$ das vezes ($h=1$) e
$98{,}3\%$ ($h=3$), contra $\approx87\%$ do modelo logístico com características --- como na
margem intensiva, as características explicam bem, mas não superam a persistência.
...
cinza: MQO enviesado, referência; vermelho: Arellano--Bond, sinal instável).}
\label{fig:lambda}
\caption*{\footnotesize Fonte: elaboração própria (script \texttt{R/38}).}
\end{figure}

Quatro leituras, visíveis na Figura~\ref{fig:lambda}. \textbf{Primeira}, o sinal e a existência de reversão ao alvo são robustos a
...
\subsubsection{Previsão recursiva e o teste de Diebold--Mariano}
...
(erro-padrão agrupado por fundo).}
\label{tab:pool}
\begin{tabular}{@{}lrr@{}}
\toprule
...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^{*}p<5\%$, $^{**}p<1\%$, $^{***}p<0{,}1\%$, com erro-padrão agrupado
por fundo (\textbf{correção desta versão}: a versão anterior reportava significância de MQO
comum, inconsistente com o restante do trabalho --- os $t$ agrupados são bem menores, mas as
conclusões qualitativas sobrevivem; ver texto). Fatores diários do NEFIN-USP (Núcleo de Economia
Financeira, USP: mercado, tamanho, valor e momento, metodologia Fama-French/Carhart adaptada ao
Brasil), agregados por retorno composto dentro do mês. Fonte: elaboração própria (script
\texttt{R/34}); fatores de \url{https://nefin.com.br/data/risk-factors/}.}
\end{table}
...
\textbf{Correção desta versão:} os erros-padrão desta tabela eram, em versões anteriores, de MQO
comum (não agrupado) --- inconsistentes com a convenção usada no resto do trabalho (Newey-West na
Seção~\ref{sec:nw}, cluster por fundo no ajuste parcial e na margem extensiva), já que
observações do mesmo fundo em meses diferentes não são independentes. Agrupando por fundo, as
estatísticas $t$ caem bastante (ex.: tamanho de $t=-19{,}0$ para $t=-3{,}1$; cotistas de
$t=30{,}9$ para $t=5{,}8$), mas \textbf{todas as conclusões qualitativas sobrevivem}: as quatro
características clássicas continuam significativas, preço e beta de VALE3 continuam estáveis e
significativos com e sem os fatores NEFIN, e valor/momento continuam não significativos. A única
mudança de leitura é a captação, que passa de não significativa para marginalmente significativa
($p<5\%$) na coluna com fatores NEFIN --- um resultado no limiar, que não muda a conclusão de que
a captação tem papel secundário na explicação do peso (Seção~\ref{sec:det}).
...
$73\%$ da amostra ($8.333$ de $11.413$ observações): fundos no início de sua vida (menos de 252
pregões de histórico de cota) ficam sem beta, uma limitação a registrar.

A Tabela~\ref{tab:betafundo} inclui o beta do fundo como regressor adicional na regressão
empilhada, com erro-padrão agrupado por fundo. Para isolar o efeito de \emph{adicionar} a
variável do efeito de \emph{restringir} a amostra, as duas colunas usam exatamente a mesma
amostra de $8.333$ observações (só os fundo-meses com beta do fundo disponível).

\begin{table}[h]\centering\small
\caption{Regressão empilhada, mesma amostra ($n=8.333$), sem e com o beta do próprio fundo.}
\label{tab:betafundo}
\begin{tabular}{@{}lrr@{}}
\toprule
Variável & Sem beta do fundo & Com beta do fundo \\
\midrule
$\ln$(patrimônio) por d.p. & $-0{,}0067$ ($t=-2{,}5$) & $-0{,}0010$ ($t=-0{,}7$) \\
$\ln$(cotistas) por d.p.   & $0{,}0125$ ($t=5{,}5$)   & $0{,}0015$ ($t=0{,}9$) \\
Fundo de cotas              & $-0{,}0202$ ($t=-3{,}6$) & $-0{,}0060$ ($t=-1{,}7$) \\
Captação/patrimônio        & $-0{,}0005$ ($t=-2{,}6$) & $-0{,}0001$ ($t=-0{,}5$) \\
Preço de VALE3 por d.p.    & $0{,}0054$ ($t=5{,}2$)   & $0{,}0032$ ($t=3{,}4$) \\
Beta de VALE3 por d.p.     & $-0{,}0038$ ($t=-3{,}8$) & $-0{,}0050$ ($t=-5{,}6$) \\
Beta do próprio fundo por d.p. & --- & $0{,}0234$ ($t=17{,}8$) \\
\midrule
$R^2$ & $0{,}130$ & $0{,}469$ \\
\bottomrule
\end{tabular}
\caption*{\footnotesize Erro-padrão agrupado por fundo. Fonte: elaboração própria (script
\texttt{R/39}).}
\end{table}

O resultado é mais forte do que uma checagem de consistência: o coeficiente do beta do fundo é
grande, positivo e fortemente significativo ($t=17{,}8$), o $R^2$ quase quadruplica
($0{,}130\to0{,}469$), e --- o achado que merece destaque --- \textbf{tamanho, cotistas, fundo de
cotas e captação perdem significância estatística} nessa amostra restrita ao serem controlados
pelo beta do fundo. A leitura mais provável é que parte do que tamanho, cotistas e FIC pareciam
explicar sobre o peso de VALE3 é, na verdade, um proxy indireto para o quão orientado a ações é o
mandato do fundo --- e o beta do fundo mede essa dimensão diretamente, absorvendo boa parte do
poder explicativo das outras características nesta subamostra. Preço e beta de VALE3 permanecem
significativos nas duas colunas, sinal de que capturam algo adicional, não redundante com o perfil
de risco geral do fundo. Essa qualificação --- válida apenas para os $73\%$ da amostra com
histórico de cota suficiente --- não estava presente em versões anteriores deste trabalho, que
citavam esse resultado sem reportar os números.

\subsection{Extensão: a demanda por um universo amplo de ações}
\label{sec:multi}
...
\subsubsection{Construção do painel amplo}

O painel fundo$\times$ativo$\times$mês (Seção~\ref{sec:dados}), já restrito a posições diretas
(444 ativos distintos), permite estimar a mesma regressão~\eqref{eq:cs}, célula a célula, para
\emph{cada} par (ativo, mês) com pelo menos 15 fundos do Itaú detentores --- um limiar que
garante graus de liberdade suficientes para os cinco parâmetros da regressão. Isso produz 13.821
células (ativo, mês), cobrindo 375 ativos distintos, 888.186 observações fundo-ativo-mês. Cada
célula gera um vetor $\widehat\theta_{n,t}$, exatamente como a cross-section mensal da
Seção~\ref{sec:cs}, mas agora indexado também pelo ativo $n$ --- é a matriz de fatores latentes
($\theta$, \emph{asset embeddings}) que o desenho teórico de Koijen--Yogo antecipa, estimada a
partir da matriz de características dos fundos ($x$, \emph{investor embeddings}), tal como
discutido com o orientador.

\subsubsection{O padrão de VALE3 generaliza?}

Para cada ativo com pelo menos 24 meses de $\widehat\theta_{n,t}$ estimado (207 ativos), calcula-se
a média temporal dos coeficientes, e observa-se a distribuição \emph{entre ativos} dessa média.

\begin{center}\scriptsize
\begin{tabular}{@{}lccc@{}}
\toprule
Coeficiente & Direção (VALE3) e \% dos 207 igual & Posição de VALE3 entre os 207 & Percentil \\
\midrule
...
\begin{figure}[h]\centering
\includegraphics[width=0.92\textwidth]{fig_multiativo_dist.pdf}
...
A resposta é matizada, e mais rica do que uma primeira leitura sugere. As \emph{direções} dos
três efeitos estruturais generalizam fortemente: 88 a 98\% dos ativos com amostra suficiente têm
o mesmo sinal que VALE3 para tamanho, cotistas e fundo de cotas --- não é uma peculiaridade de
VALE3, é um padrão de como os fundos do Itaú, em geral, distribuem posições em ações conforme o
perfil do fundo. Mas a \emph{magnitude} desses efeitos para VALE3 é extrema em \textbf{três} das
quatro dimensões, não apenas na de tamanho: o coeficiente de tamanho é o 3º mais negativo entre
207 ativos (percentil $1{,}4$), o de cotistas é o 2º mais positivo (percentil $99{,}5$), e o de
fundo de cotas é o 2º mais negativo (percentil $1{,}0$) --- visíveis na Figura~\ref{fig:multiativo}
como a linha vermelha na cauda extrema de três dos quatro histogramas. Só o coeficiente de fluxo
é comum (percentil $61$, sem destaque). Isso é uma qualificação importante para a leitura de todo
o restante deste trabalho: os números de \emph{magnitude} estimados para VALE3 (os pontos
percentuais específicos citados nas Seções~\ref{sec:det} e~\ref{sec:fraclogit}) não devem ser
extrapolados, sem ressalva, como ``o efeito típico de uma ação qualquer'' --- são o efeito para um
papel particular, grande, líquido e historicamente volátil, que se encontra reiteradamente nas
caudas dessa distribuição. As \emph{direções} dos efeitos, essas sim, generalizam.

\subsubsection{Regressão pooled com efeito-fixo de ativo e de mês}

Como checagem complementar, a Tabela~\ref{tab:fe2way} reporta uma única regressão empilhando
todas as 894.551 observações do painel amplo, com efeito-fixo de \emph{ativo} e de
...
\begin{table}[h]\centering\small
\caption{Regressão pooled multiativo com efeito-fixo de ativo e de mês (894.551
observações, 444 ativos, 305 fundos, 72 meses).}
\label{tab:fe2way}
\begin{tabular}{@{}lrrr@{}}
\toprule
Variável & Coeficiente & Erro padrão (cluster fundo) & $t$ \\
\midrule
$\ln$(patrimônio) por d.p. & $-0{,}00125$ & $0{,}00038$ & $-3{,}3$ \\
$\ln$(cotistas) por d.p.   & $0{,}00224$  & $0{,}00028$ & $8{,}1$ \\
Fundo de cotas             & $-0{,}00276$ & $0{,}00074$ & $-3{,}7$ \\
Captação/PL                & $0{,}00003$  & $0{,}00007$ & $0{,}4$ \\
\bottomrule
\end{tabular}
...
\subsubsection{Limitações explícitas desta extensão}

Mesmo depois da limpeza que reduziu o painel a posições diretas (444 ativos, todos com
\emph{ticker} padrão da B3), a parte \emph{dinâmica} do modelo (preço e beta próprios de cada
ativo, filtro de Kalman, ajuste parcial, previsão fora da amostra) não foi estendida a todos os
papéis nesta rodada --- isso exigiria buscar preço e beta individualmente para cada um dos 444
\emph{tickers} em uma fonte de mercado, o que é mecanicamente mais tratável agora que os nomes
estão padronizados (cada ativo já tem um \emph{ticker} B3 identificável, ao contrário do painel
bruto original, em que ``VALE3'' aparecia sob rótulos ambíguos como posição emprestada ou direito
de subscrição), mas ainda não foi feito por tempo. VALE3 permanece, portanto, o estudo de caso
aprofundado para a parte dinâmica; a generalização desta seção cobre apenas a camada de
\emph{explicação} transversal (o primeiro passo do modelo em dois passos, Seção~\ref{sec:cs}),
não a segunda. Estender a cobertura de preço/beta aos 444 ativos --- ou, para começar, aos mais
frequentemente detidos --- fica registrado como agenda futura (Seção~\ref{sec:agenda}), agora
mais tratável do que antes da limpeza.
...
foco aprofundado em VALE3 e, nesta versão, extensão a um universo de 375 ações --- e o
...
longo de 72 meses \emph{para VALE3}, e que generalizam na mesma direção para 88 a 98\% de um
universo amplo de ativos --- não é um artefato de um único papel. Ao mesmo tempo, a magnitude do
efeito de VALE3 é extrema em três das quatro dimensões (tamanho, cotistas e fundo de cotas) dentro
dessa distribuição, uma ressalva que qualifica quanto os números específicos de VALE3 podem ser
generalizados. A decisão de ter a
...
exclusivos é uma convenção cuja sensibilidade não foi testada; a elasticidade-preço no sentido
original de Koijen--Yogo não foi estimada; e a comparação com/sem beta do fundo
(Seção~\ref{sec:betafundo}) usa apenas os $73\%$ da amostra com histórico de cota suficiente para
estimar esse beta ($\geq252$ pregões), uma restrição sistemática --- exclui fundos mais novos, não
aleatoriamente --- cuja sensibilidade também não foi testada.
