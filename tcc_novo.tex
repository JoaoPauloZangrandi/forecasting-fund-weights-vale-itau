...
$0{,}152$. Os erros padrão estão corrigidos para autocorrelação pelo método de Newey-West
(defasagem $L=3$), conforme a Seção~\ref{sec:nw}; o cálculo clássico daria erros cerca de $1{,}7$
a $1{,}8$ vez menores. Três asteriscos indicam significância a um por mil. Fonte: elaboração
própria (script \texttt{R/24}).}
\end{table}

Convém quantificar a magnitude dos efeitos, e não apenas seus sinais, lendo-os por desvio padrão
das características de escala.\footnote{As magnitudes por desvio padrão vêm de reestimar a regressão
com o logaritmo do patrimônio e o logaritmo do número de cotistas padronizados; os coeficientes da
Tabela~\ref{tab:fmtab} permanecem na escala original de log. Execução no script \texttt{R/24}.}
Com as características de escala centradas, o intercepto passa a cerca de $0{,}040$, isto é, um peso
de referência em torno de 4 por cento para um fundo de tamanho médio que não seja de cotas ---
recuperando, como observado na Seção~\ref{sec:cs}, o nível médio do peso. Um aumento de um desvio
...
estatísticas em torno de 11 a 12,5 em valor absoluto após a correção (contra 19 a 22 no cálculo
...
Os resultados confirmam a leitura linear: os sinais e a significância das características são
idênticos, e os efeitos parciais médios --- a resposta em pontos de peso, comparável aos
coeficientes lineares --- praticamente coincidem, com cerca de $-0{,}68$ ponto percentual para o
patrimônio, $+1{,}06$ para o número de cotistas e $-1{,}60$ para fundo de cotas, por desvio padrão.
