...
A regressão transversal foi também reestimada com a ligação logística da equação~\eqref{eq:target},
um modelo de resposta fracionária apropriado a uma variável limitada (Papke e Wooldridge, 1996).
Os resultados confirmam a robustez da leitura anterior: os sinais e a significância das
características são idênticos aos da versão linear, e os efeitos no centro da distribuição
praticamente coincidem. A diferença prática aparece nos extremos. A regressão linear gera pesos
ajustados fora do intervalo entre zero e um em cerca de dois e meio por cento das observações,
valores sem sentido para uma participação, ao passo que a logística os respeita por construção.
Por isso a forma logística é adotada como a especificação correta do peso desejado, e é ela que
alimenta o modelo de ajuste parcial.
