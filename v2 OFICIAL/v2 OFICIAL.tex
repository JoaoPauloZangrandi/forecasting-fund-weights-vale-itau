...
Em pontos percentuais (mais fáceis de ler diretamente), a versão \textbf{linear} da mesma
comparação mostra o mesmo padrão de enfraquecimento: $\alpha=0{,}0355$ (era $0{,}0452$),
$\theta^{aum}=-0{,}0016$ *** (era $-0{,}0074$ ***), $\theta^{cot}=0{,}0018$ *** (era $0{,}0130$ ***),
$\theta^{fic}=-0{,}0057$ *** (era $-0{,}0196$ ***), $\theta^{bf}=0{,}0233$ ***, $R^2=0{,}532$ (era
$0{,}166$).
...
$$\text{peso}_{i,t} = \alpha_t + \theta^{aum}_t z(\ln AUM_i) + \dots + \theta^{bf}_t z(\beta^{fundo}_i)
...
$\theta^{aum}$ (tamanho)        & $-0{,}00178$ & *** & $-0{,}00171$ & *** \\
$\theta^{cot}$ (cotistas)       & $0{,}00236$  & *** & $0{,}00118$  & *** \\
$\theta^{fic}$ (fundo de cotas) & $-0{,}00681$ & *** & $-0{,}00621$ & *** \\
$\theta^{flow}$ (captação/resgate) & $-0{,}00446$ & *** & $-0{,}00312$ & ** \\
$\theta^{bf}$ (beta do fundo)   & $0{,}02536$  & *** & $0{,}02528$  & *** \\
\midrule
...
\caption{Evolução mensal do coeficiente de gestora (vs.\ Itaú), para as 5 gestoras com coeficiente
mais positivo e as 5 com coeficiente mais negativo \textbf{de verdade} (mesmo ranking da
Tabela~7, sem filtro de cobertura). Duas delas têm pouquíssimos meses --- Legacy Capital ($1$ mês,
aparece como ponto isolado) e Núcleo Capital ($4$ meses) --- porque são, de fato, as gestoras com
o coeficiente médio mais alto; não há ``evolução'' de verdade pra mostrar nesses dois casos
específicos, só o valor pontual. O pico da Constellation em mar/2019 é o mesmo padrão de sempre:
naquele mês só havia $1$ fundo da casa na amostra. Já o pico da Occam Brasil em 2021 \textbf{não}
é um artefato de grupo pequeno --- tinha $20$ fundos naquele mês, com pesos variados (vários
entre $16\%$ e $16{,}4\%$) --- reflete uma mudança de alocação mais ampla da casa, não investigada
em detalhe aqui.}
\end{figure}
...
dispersas de verdade (mesmo ranking da Tabela~8, sem filtro de cobertura --- por isso o painel de
baixo tem gestoras com cobertura mais curta, $11$ a $33$ dos $59$ meses). O pico isolado da Vinci
Partners em 2021 segue o mesmo padrão já visto na evolução dos coeficientes de gestora: grupo
pequeno, sensível a um único fundo concentrado naquele mês.}
\end{figure}
...
\equacao
$$\text{peso}_{i,n,t} = \alpha_{n,t} + \theta^{aum}_{n,t}z(\ln AUM_i) + \theta^{cot}_{n,t}z(\ln\text{cot}_i)
+ \theta^{fic}_{n,t}FIC_i + \theta^{flow}_{n,t}(\text{fluxo/AUM})_i + \theta^{bf}_{n,t}z(\beta^{fundo}_i) + \text{erro}$$
