...
par(mfrow = c(4,2), mar = c(3,4,2.8,1), cex.main = 1.15, cex.axis = 0.95, cex.lab = 0.95)
...
par(mar = c(4,4,1,1))
hist(e_ind$e, breaks = 80, col = "#2E5C8A", border = "white", main = "",
...
plot(datas_ind, e_ind$e, pch=16, cex=0.35, col=rgb(0.4,0.4,0.4,0.18), ylim=yl,
     xlab="", ylab="erro (fração do peso)", main="")
...
par(mar = c(4,4,1,1))
hist(u_ind$u, breaks = 80, col = "#2E5C8A", border = "white", main = "",
...
bp <- barplot(mat, beside = TRUE, col = c("#2E5C8A","#B0B8C1"),
              names.arg = c("Treino\n(2017-2019)","Teste\n(2020-2021)"),
              ylab = "RMSE", main = "",
              ylim = c(0, max(mat)*1.35))
text(bp, mat, sprintf("%.4f", mat), pos = 3, cex = 0.75)
legend("top", legend = c("Ajuste parcial (lambda de treino)","Ingênua"),
       fill = c("#2E5C8A","#B0B8C1"), bty = "o", bg = "white", box.col = "grey85",
       horiz = TRUE, cex = 0.8)
dev.off()
