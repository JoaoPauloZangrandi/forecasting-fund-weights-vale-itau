# =============================================================================
# 52_features_universo_completo.R  (v2 OFICIAL -- teste_estrategia)
#
# FASE 0, passo 3: AUM/cotistas/fluxo predeterminados (t-1, via Informe
# Diario) pro universo COMPLETO de 3.254 fundos (R/50) -- mesma logica do
# R/18, so trocando a lista de fundos-alvo (2.821 -> 3.254). O custo extra
# de ler mais 434 CNPJs nos mesmos arquivos mensais e' pequeno (o grosso do
# tempo e' ler o arquivo inteiro, nao filtrar).
...
painel <- fread(file.path(REPO, "v2 OFICIAL/data/painel_multiativo_direto_completo.csv"))
painel[, ym := ano*100L + mes]
cnpjs <- unique(painel$cnpj)
cat("Fundo-meses no painel (universo completo):", nrow(painel), "| CNPJs distintos:", length(cnpjs), "\n")

addm <- function(ym, k) { tot <- (ym %/% 100L)*12L + (ym %% 100L - 1L) + k; (tot %/% 12L)*100L + (tot %% 12L) + 1L }
base_fm <- unique(painel[, .(cod_fundo, cnpj, ano, mes, ym)])
base_fm[, ym_prev := addm(ym, -1L)]
meses_prev <- sort(unique(base_fm$ym_prev))
...
painel2 <- merge(base_fm, prev, by.x = c("cnpj","ym_prev"), by.y = c("CNPJ_FUNDO","ymk"), all.x = TRUE)
...
# ---- FIC vs FI pelo nome (mesmo criterio do R/18/04) -----------------------
nomes <- fread(file.path(REPO, "v2 OFICIAL/data/universo_completo_gestora.csv"))[, .(cod_fundo, nome_fundo)]
painel2 <- merge(painel2, nomes, by = "cod_fundo", all.x = TRUE)
...
fwrite(painel2, file.path(REPO, "v2 OFICIAL/data/features_predeterminado_completo.csv"))
cat("\nOK - salvo em 'v2 OFICIAL/data/features_predeterminado_completo.csv'\n")
