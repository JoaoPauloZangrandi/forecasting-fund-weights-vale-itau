...
nos 72 meses, e que generalizam para 92--99\% de quase 900 ações), mas o peso de cada fundo é tão
...
generalização da estimação de demanda para um \textbf{universo amplo de quase 900 ações}
...
O painel resultante --- fundo $\times$ ativo $\times$ mês --- tem \textbf{1.777 ativos distintos
brutos}, 906 fundos, quase 7,8 milhões de linhas (2016--2021). Após limpeza (10 linhas com nomes
de "Ação de Companhia Fechada" continham vírgula que corrompia o parsing CSV bruto da CVM, e mais
3.293 linhas do mesmo tipo de ativo --- fora de escopo, sem preço de mercado --- foram descartadas
com auditoria) e o filtro de mais de três cotistas, sobram 1,50 milhão de observações, 305 fundos,
1.129 ativos. Uma checagem de consistência forte: VALE3 tem exatamente 26.123 linhas nesse painel
amplo --- o mesmo número do painel original, construído por um caminho de código totalmente
diferente.
...
(7.964 deles), não linhas do painel amplo.
...
a mesma cross-section, célula por célula: 23.704 células, 868 ativos distintos, 1,49 milhão de
observações, em 33 segundos de computação. \scriptref{R/32}

\resultados Para cada ativo com pelo menos 24 meses de $\widehat\theta_{n,t}$ estimado (361
...
\begin{tabular}{@{}lccc@{}}
\toprule
Coeficiente & Direção (VALE3) e \% igual & Posição de VALE3 entre os 361 & Percentil \\
\midrule
$\ln$(PL)         & negativo, $92{,}5\%$ & \textbf{3º mais negativo} & $0{,}8$ \\
$\ln$(cotistas)   & positivo, $98{,}9\%$ & \textbf{2º mais positivo} & $99{,}7$ \\
Fundo de cotas     & negativo, $92{,}8\%$ & \textbf{2º mais negativo} & $0{,}6$ \\
Captação/PL       & fraco/instável, $52{,}6\%$ positivo & comum, sem destaque & $76{,}2$ \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{Distribuição, entre os 361 ativos com amostra suficiente, do coeficiente médio de cada
...
generalizam fortemente: 92 a 99\% dos ativos com amostra suficiente têm o mesmo sinal que VALE3
...
primeira checagem sugeria: o coeficiente de tamanho é o 3º mais negativo entre 361 ativos
(percentil $0{,}8$), o de cotistas é o 2º mais positivo (percentil $99{,}7$), e o de fundo de
cotas é o 2º mais negativo (percentil $0{,}6$) --- visíveis na Figura~\ref{fig:multiativo} como a
linha vermelha na cauda extrema de três dos quatro histogramas. Só o coeficiente de fluxo é comum
(percentil $76$). VALE3, portanto, não é um caso representativo do universo de ações que os
fundos do Itaú carregam --- é um papel grande, líquido e historicamente volátil que puxa para o
extremo em quase toda dimensão medida, e os números de \emph{magnitude} estimados para ele ao
longo deste documento não devem ser lidos como ``o efeito típico de uma ação qualquer''. Uma
checagem independente, empilhando as 1,50 milhão
de observações com efeito-fixo de \emph{ativo} \textbf{e} de \emph{mês} (estimado por
demeaning iterativo de Frisch--Waugh--Lovell, que subtrai repetidamente as médias por ativo e por
mês até convergir --- equivalente a incluir uma \emph{dummy} para cada um dos 1.129 ativos e cada
um dos 72 meses, mas computacionalmente muito mais barato), erro-padrão agrupado por fundo,
confirma os três sinais com significância ($t=-3{,}4$; $t=8{,}3$; $t=-4{,}0$; $t=0{,}6$ para o
fluxo, não significativo). \scriptref{R/32}

\limitacoes O universo bruto (1.777 ativos) é grande demais para estender a parte
\textbf{dinâmica} do modelo (preço e beta próprios de cada ativo, Kalman, ajuste parcial,
previsão fora da amostra) a todos os papéis nesta rodada: mapear cada nome de ativo para um
\emph{ticker} de mercado e buscar preço/beta individualmente não é confiável de fazer em massa
(nomes ambíguos, BDRs, ativos pouco líquidos, limites de taxa de fontes externas). VALE3
permanece o estudo de caso aprofundado para a parte dinâmica; esta seção cobre só a camada de
\emph{explicação} transversal (o Passo 1). Estender a cobertura de preço/beta a mais ativos ---
por exemplo, aos mais frequentemente detidos --- fica na agenda.
...
Preço/beta só de VALE3 no painel amplo (1.777 ativos) & priorizar ativos mais frequentes & agenda \\
...
versão --- generaliza em \textbf{direção} para 92 a 99\% de quase 900 outras ações que os fundos
do Itaú carregam. A \textbf{magnitude} dos efeitos de VALE3, porém, é atípica em três das quatro
dimensões (tamanho, cotistas e FIC, todas no 1--2\% mais extremo da distribuição entre ativos):
é um papel extremo nessas dimensões, não representativo.
