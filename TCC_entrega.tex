...
% Paginacao conforme a NBR 14724: algarismos arabicos no canto superior direito,
% contando a partir da folha de rosto e aparecendo a partir da parte textual.
\pagestyle{fancy}
\fancyhf{}
...
{\Large Um estudo de previsão de carteiras de fundos de investimento}
...
\\[10pt]
Campo de conhecimento: Economia Financeira.
\\[10pt]
...
%==================== FOLHA DE APROVAÇÃO ====================
% Elemento obrigatorio pela NBR 14724, item 4.2.1.3.
\begin{center}
{\large João Paulo Zangrandi}
...
\noindent \rule{2cm}{0.4pt}\ /\ \rule{2cm}{0.4pt}\ /\ \rule{2.4cm}{0.4pt}
...
\noindent \rule{0.62\textwidth}{0.4pt}\\
Prof.\ Maurício Ferraresi Junior (Orientador)\\
FGV EESP
...
sobre seis características observáveis do fundo: o tamanho, o número de cotistas,
...
\begin{singlespace}
\noindent
This work proposes and tests a dynamic factor model for the weight of individual stocks in
the portfolios of Brazilian investment funds, organised in three stages. In the first, the
weight held by each fund in each stock is estimated, month by month and stock by stock, by a
logistic regression on six observable fund characteristics: size, number of shareholders,
fund-of-funds status, net flow over assets, the beta of the fund unit price and the
concentration of the remainder of the portfolio. In the second, a partial adjustment
mechanism models the speed at which the observed position converges to the target estimated
in the first stage. In the third, the adjustment parameter is re-estimated using only the
months prior to January 2020 and then applied, held fixed, to the subsequent period, so as to
measure the predictive value of the model on data it did not see during estimation. The
sample covers the entire universe of funds registered with the Brazilian Securities and
Exchange Commission holding equity positions between 2016 and 2021, from which a final sample
of 2,507 funds, 41 asset management groups and 512 stocks is obtained, with 8,212,294
fund-stock-month observations. The number of shareholders, the beta of the fund unit price
and the concentration of the remainder of the portfolio are the characteristics most
consistently associated with the weight. The estimated adjustment speed is positive and
small, at 0.069 per month, and the partial adjustment model outperforms the naive forecast of
repeating the last observed weight at all four horizons tested, by a margin that grows with
the horizon. The out-of-sample error is heterogeneous across asset managers, and this
heterogeneity is associated above all with portfolio concentration, which remains
statistically significant even when the remaining characteristics of the model are
controlled for.

\smallskip

\noindent Keywords: demand system; mutual funds; portfolio weights; dynamic factor; partial
adjustment; Brazilian equity market.
\end{singlespace}
...
é modelado por regressão logística sobre seis características do fundo: o tamanho, o
número de cotistas, a condição de fundo de investimento em cotas, o fluxo de captação
líquida sobre o patrimônio, o beta da cota e a concentração do restante da carteira. Essa regressão é estimada célula a célula, isto é, separadamente para cada par de
...
No exercício de explicação, as características funcionam bem: o número de cotistas, o
beta da cota e a concentração do restante da carteira são as mais consistentes em sinal ao
longo das mais de treze mil células estimadas. No de previsão, o ajuste parcial supera a
previsão ingênua nos quatro horizontes testados, com margem que
...
Os dados de fundos utilizados neste trabalho foram obtidos por meio da base Economatica, em
extração repassada pelo orientador e referente ao período de janeiro de 2016 a dezembro de
2021. A origem primária do dado é regulatória: a Economatica compila dois registros que a
Comissão de Valores Mobiliários exige de todo fundo de investimento brasileiro. O primeiro é
a Composição e Diversificação de Aplicações, extrato mensal do qual se extrai o peso de cada
ativo na carteira de cada fundo, que é a variável dependente deste trabalho. O segundo é o
Informe Diário, do qual vêm o patrimônio líquido, o número de cotistas, a captação e os
resgates, que compõem as características do fundo. O acesso é, portanto, indireto, embora o
conteúdo seja o do registro público.

O que a base comercial acrescenta ao registro bruto é relevante para o desenho deste
trabalho e justifica a escolha. Em primeiro lugar, a composição das carteiras vem
consolidada, isto é, com a exposição dos fundos de investimento em cotas atribuída aos
ativos finais em vez de às cotas dos fundos investidos. Essa consolidação não é um detalhe de
conveniência: no mercado brasileiro a estrutura de fundo de cotas sobre fundo master é a
regra, e uma reconstrução a partir do registro bruto mostra que, sem a atribuição aos ativos
finais, apenas cerca de 19\% dos fundos que reportam posição em ações apresentam essa posição
na própria carteira. Sem consolidação, a variável dependente seria nula para a maior parte da
amostra, e as características do fundo estariam medidas num veículo distinto daquele em que a
carteira é observada. Em segundo lugar, a base traz a identificação da gestora por marca
controladora e a classificação da Associação Brasileira das Entidades dos Mercados Financeiro
e de Capitais, que não constam do registro público de forma diretamente utilizável.

Duas fontes complementares foram acrescentadas. O índice Bovespa, em série diária de
fechamento, é utilizado como carteira de mercado na estimação do beta da cota de cada fundo,
descrito adiante. E os dois registros da Comissão foram reconstruídos de forma independente,
a partir do dado público bruto, para verificar a base recebida. Essa verificação mostra
correspondência praticamente exata para as variáveis do Informe Diário, com 99\% ou mais de
identidade para cota, número de cotistas e patrimônio líquido, e correspondência parcial para
a composição de carteira, com erro mediano por fundo entre 9\% e 18\% conforme o mês testado.
Esse resíduo decorre de procedimentos proprietários de reconciliação da base comercial que
não são reproduzíveis a partir do dado bruto, e é registrado aqui como uma característica
conhecida da fonte.

O universo de partida é todo fundo registrado na Comissão que tenha mantido posição em ações
no período, o que totaliza 3.254 fundos e 41 grupos de gestora reunidos por marca
controladora. Não há, portanto, restrição prévia a uma gestora ou a um ativo específico.
...
do código de negociação segundo a convenção da bolsa, o que resulta em 536 ativos e em
10.818.395 linhas de fundo por ativo e por mês antes dos demais filtros. Vinte e seis dos
3.254 fundos do universo de partida não registram nenhuma posição direta em ação no período,
apenas posição em empréstimo ou direito de subscrição, e por isso não aparecem nessas linhas,
de modo que o painel de posição direta reúne 3.228 fundos.
...
\midrule
Universo bruto com posição em ações, 2016 a 2021 & 3.254 & n.a. & 41 \\
Com ao menos uma posição direta em ação & 3.228 & 536 & 41 \\
...
\end{tabular}
\caption*{\footnotesize Fonte: elaboração própria a partir de dados da Comissão de Valores
Mobiliários. A sigla n.a. indica que a contagem de ativos não se aplica à primeira
linha, porque um ativo só passa a ser identificado depois do filtro de posição direta. A cobertura temporal da amostra final é
de 59 meses, entre fevereiro de 2017 e dezembro de 2021, e não os 72 meses do período bruto,
porque a exigência de 252 pregões de histórico de cota para o cálculo do beta elimina os
primeiros meses da janela.}
\end{table}
...
é mecanicamente nula em cada célula, o que constitui uma propriedade algébrica do
estimador e nada informa sobre a qualidade do ajuste. O que de fato varia entre fundos, e é informativo, é a
sua dispersão.
...
interpretável, que é o número de meses que um fundo leva para percorrer metade da distância
até o seu peso-alvo.
...
médio em pontos percentuais de peso, ambos em mediana entre as células, e a proporção de
células em que o sinal é positivo.
...
\begin{tabular}{@{}lrrr@{}}
\toprule
Característica & Mediana do & Mediana do efeito & Células com \\
 & coeficiente & marginal médio & sinal positivo \\
\midrule
...
\end{tabular}
\caption*{\footnotesize Fonte: elaboração própria a partir de dados da Comissão de Valores
Mobiliários. O coeficiente é medido em log-odds sobre a característica padronizada. O efeito
marginal médio é calculado dentro de cada célula, como a média do efeito parcial sobre os
fundos daquela célula, e a tabela reporta a mediana desses valores entre as 13.723 células,
pela mesma razão que leva a coluna anterior a reportar a mediana e não a média: a distribuição
entre células tem cauda longa, e algumas poucas células extremas deslocariam a média. O
...
O padrão encontrado admite leitura econômica direta. O beta da cota e a concentração do resto da
...
curtose muito altas decorrem de a base reunir 512 ativos de liquidez e perfil muito
diferentes numa única distribuição agregada, dominada por
...
Razão entre as duas & $0{,}9848$ & $0{,}9686$ & $0{,}9522$ & $0{,}9291$ \\
...
\caption*{\footnotesize Fonte: elaboração própria a partir de dados da Comissão de Valores Mobiliários.}
\end{figure}
...
\begin{figure}[H]\centering
\caption{Evolução mensal do erro médio fora da amostra no horizonte de um mês, para as três gestoras de maior e as três de menor erro.}
...
uma posição em Equatorial Energia que se mantém entre 96,9\% e 99,9\% do patrimônio ao longo de
...
\begin{figure}[H]\centering
\caption{Peso observado e peso previsto no horizonte de três meses para o par de fundo e ativo de maior erro fora da amostra entre os que mantêm posição relevante.}
...
Figura~\ref{fig:persistencia-dp}. Quem é difícil de prever numa metade do
...
\begin{figure}[H]\centering
\caption{Desvio-padrão do erro fora da amostra na primeira metade do período de teste, em 2020, contra a segunda metade, em 2021, por gestora. A linha tracejada é a bissetriz.}
...
\begin{figure}[H]\centering
\caption{Desvio-padrão do erro fora da amostra por gestora contra o beta médio, o tamanho médio e a concentração média da carteira, em 40 gestoras.}
...
O resultado está na Tabela~\ref{tab:regressao-variancia}. A concentração média é a
única variável estatisticamente significativa, a 1\%, e o modelo explica praticamente metade
da variação do erro entre gestoras, ou dois quintos dela quando o ajuste é penalizado pelo
número de regressores. O fato de a concentração já estar formalmente na primeira etapa não
elimina o seu poder explicativo aqui, porque as duas regressões respondem a
...
$R^2$ ajustado & \multicolumn{3}{r}{$0{,}405$} \\
\bottomrule
\end{tabular}
\caption*{\footnotesize Fonte: elaboração própria a partir de dados da Comissão de Valores
Mobiliários. A sigla n.s. indica coeficiente não significativo ao nível de 5\%. Com seis
regressores e apenas 40 gestoras, a distância entre o $R^2$ e o seu ajustado é grande, razão
pela qual os dois são reportados.}
\end{table}
...
Por fim, o ranking de dificuldade por gestora é estável entre horizontes, como
...
identificação de quais gestoras são mais difíceis de prever capta uma característica estável
de cada casa, e não um artefato do horizonte escolhido.

\begin{figure}[H]\centering
\caption{Estabilidade do ranking de dificuldade por gestora entre os quatro horizontes, em 40 gestoras com dado completo.}
\label{fig:ranking}
...
bruto ainda maior, de 3.254 fundos e de 536 ativos em posição direta, e não de um recorte
restrito a uma única gestora ou a um único papel.
...
quadráticos médios caindo de 0,9848 em um mês para 0,9291 em doze meses, e vence em todos os
...
O desenho tem limitações que precisam ser registradas. A primeira é que parte
substancial da variância do erro fora da amostra permanece sem explicação pelas seis características observáveis, uma vez que
...
de 2017 a dezembro de 2021, corresponde à cobertura da extração utilizada e descrita na
Seção~\ref{sec:dados}, de modo que estender a análise a um período mais recente fica como
trabalho futuro. A terceira diz respeito à identificação: o
...
Algumas extensões decorrem diretamente do que se encontrou aqui. A primeira é permitir que
a velocidade de ajuste varie entre gestoras ou entre classes de fundo, em vez de impor um
único parâmetro comum, o que os resultados de heterogeneidade da
Seção~\ref{sec:resultados} sugerem ser empiricamente relevante. A segunda é
...
\addcontentsline{toc}{section}{Declaração sobre o uso de inteligência artificial}

Ferramentas de inteligência artificial generativa foram utilizadas para auxiliar na revisão do
texto e na codificação computacional. Todas as sugestões e códigos gerados foram revisados e
validados pelo autor, que assume integral responsabilidade pelo conteúdo final.
